...
def view_courses():
    if session.get('role') == 'Admin':
        query = """
        SELECT c.course_id, c.course_name, d.department_name, c.course_target, CONCAT(u1.name, ' (', u1.role, ')') AS added_by, CONCAT(u2.name, ' (', u2.role, ')') AS updated_by FROM courses c JOIN departments d ON c.department_id = d.department_id LEFT JOIN (select * from users where role!='Student') u1 ON c.target_added_by = u1.user_id LEFT JOIN (select * from users where role!='Student') u2 ON c.target_updated_by = u2.user_id
        """
...
def view_faculty():
    if session.get('role') == 'Admin':
        ...
        FROM faculty f
        JOIN (select * from users where role not in('Admin','Student')) u ON f.user_id = u.user_id
...
def view_student():
    if session.get('role') == 'Admin':
        ...
        FROM students s
        JOIN (select * from users where role not in('Admin','Faculty')) u ON s.user_id = u.user_id
