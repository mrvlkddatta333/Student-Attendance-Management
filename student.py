...
def check_in(course_id):
    ...
    if timetable is None:
        flash("Class Time not Set by Faculty!")
        return redirect(url_for("student.view_enrolled_courses"))
...
def view_attendance(course_id):
    ...
    if course_details is None:
        flash("Faculty is not yet assigned to this Course!")
        return redirect(url_for('student.view_enrolled_courses'))
    ...
    if start_date is None:
        flash("Course Start Date is not set by Faculty!")
        return redirect(url_for('student.view_enrolled_courses'))

    if target_days is None:
        flash("Course Duration is not set by Faculty!")
        return redirect(url_for('student.view_enrolled_courses'))
